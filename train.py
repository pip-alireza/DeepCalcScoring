...
for msk in sorted(glob.glob(path +  '/*mask.png')):
    ...
    mask = mask[:, :, 0] # for mask it is 3 dimension image (512,512,3) but all 3 are same. The purpose i to use pretrained "ImageNet" weight
...
model = Model(BACKBONE,input_shape=(512, 512, 3), encoder_weights='imagenet', classes=1, activation='sigmoid')
model.summary()
